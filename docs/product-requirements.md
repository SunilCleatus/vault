# Vault — Product Requirements Document (v0.1 / MVP)

## 1. Summary

Vault is a mobile app for securely storing and organizing personal identity, legal, and family documents (photos/scans of licenses, property papers, Aadhaar, PAN, insurance, etc.) in a folder structure on the user's own Google Drive. The user owns the storage (their Drive quota, their account); the app is the organized, secure front door to it.
...
- iOS support (decide platform priority in section 10).
- Multi-cloud support (Dropbox, OneDrive, iCloud) — Drive only for v1.

## 4. Target Users

- Primary: Individuals/heads of household in India (or similar geographies with Aadhaar/PAN-style national IDs) who manage documents for themselves and their family.
- Secondary: Family members with shared or delegated access (spouse, adult children) — post-MVP.
...
### 6.8 Security
- App-level lock: biometric (fingerprint/Face unlock) or PIN required to open the app, independent of phone lock.
- Locally cached/favorited files encrypted at rest on device.
- No document content ever touches Vault's own servers — Drive API calls go directly device → Google (thin/no backend for document data in MVP).
...
- **Platform**: Android first (largest addressable base in target geography, simpler storage permission model for camera-heavy apps). iOS as fast-follow.
...
| Family member documents — do family members get their own login, or is this single-owner-manages-all for MVP? | MVP assumption: single account manages all family members' docs (like a folder, not multi-user accounts). Multi-user shared access is a v2 consideration. |
| Offline favorites encryption — what's the key management approach (device keystore vs. passphrase)? | Recommend Android Keystore-backed encryption; no separate passphrase to manage for MVP. |
...
1. **M1 — Auth + Folder Scaffolding**: Google Sign-In, Drive `drive.file` OAuth, auto-create default folder structure.
2. **M2 — Capture & Upload**: Camera capture with cropping, multi-page PDF, category picker, resumable upload to Drive.
3. **M3 — Browse & Viewer**: Category grid, family member switcher, in-app image/PDF viewer, local metadata cache.
4. **M4 — Search & Security**: Global search, app lock (biometric/PIN), encrypted offline favorites.
5. **M5 — Reminders & Polish**: Expiry reminders, import-existing-files flow (Drive Picker), onboarding, empty states.

## 12. Open Decision for the User

Before implementation starts, please confirm:
- Target platform priority: Android-first (recommended) vs. cross-platform framework (e.g., React Native/Flutter) from day one?
- Should family member documents be filterable/shareable with those family members directly in v1, or is "single owner manages all" acceptable for MVP (as assumed above)?
